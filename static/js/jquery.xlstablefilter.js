      ...
      *****************/
		_create: function() {
			...
			$.each(headers, function(i, h) {
				...
				if (filterable == true) {
					$(this).addClass("xlsFilterHeader").dblclick(function(e) {
		...
		****************************/
		...
		_getFilterContent: function(header) {
			...
			$.each($(self.element).find("tr td:nth-of-type(" + colNum + ")"), function(i, el) {
				vals[vals.length] = $(el).find('input[type="text"]').val() || $(el).find('option:selected').text();
			});
			vals = this.sortUnique(vals);
			
			var filter = '<form id="xlsFilterForm">';
			filter = filter + '<div>搜索： <input type="text" id="filterSearch" style="width: 200px;border:1px solid #ccc;"></div>';
			filter = filter + '<div id="xlsFilterAllNone"><span id="xlsFilterAll">全选</span> | <span id="xlsFilterNone">取消</span></div>';
